...
## Rate Limits
- Login: 429 when the credential rate limiter is exceeded. Limited per username (defaults 5 per minute and 15 per 24 hours, env vars `LOGIN_LOWER_LIMIT_MSECS`, `LOGIN_LOWER_LIMIT_COUNT`, `LOGIN_UPPER_LIMIT_MSECS`, `LOGIN_UPPER_LIMIT_COUNT`; both buckets count every attempt, successful logins included) and per source IP (defaults 60 per minute and 2000 per 24 hours, env vars `LOGIN_IP_LOWER_LIMIT_MSECS`, `LOGIN_IP_LOWER_LIMIT_COUNT`, `LOGIN_IP_UPPER_LIMIT_MSECS`, `LOGIN_IP_UPPER_LIMIT_COUNT`). The per-IP limits are deliberately generous so many users behind one NAT don't trip them.
