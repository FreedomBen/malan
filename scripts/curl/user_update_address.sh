...
NEW_EMAIL="NewSuperUser9@example.com"
NEW_USERNAME="NewSuperUser9"
...
NEW_ADDRESSES='[{"city":"some city","country":"some country","name":"Jared","line_1":"l1","line_2":"l2","postal":"po","primary":false,"state":"ST"}]'
...
curl \
  ...
  --header "Content-Type: application/json" \
  --header "Authorization: Bearer ${api_token}" \
  --data "{\"user\":{\"roles\":${NEW_ROLES},\"phone_numbers\":${NEW_PHONE_NUMBERS},\"addresses\":${NEW_ADDRESSES}}}" \
...
curl \
  ...
  --header "Content-Type: application/json" \
  --header "Authorization: Bearer ${api_token}" \
  "http://localhost:4000/api/users/${user_id}" | jq
