defmodule MalanWeb.PhoneNumberControllerTest do
  ...
  describe "show" do
    ...
    test "requires authentication", %{conn: conn} do
      conn = get(conn, Routes.phone_number_path(conn, :show, "42"))
      assert conn.status == 403
    end
    ...
    test "requires accepting ToS and PP", %{conn: conn} do
      {:ok, user, session} = Helpers.Accounts.regular_user_with_session()
      conn = Helpers.Accounts.put_token(conn, session.api_token)
      conn = get(conn, Routes.phone_number_path(conn, :show, user.id))
      # We haven't accepted the terms of service yet so expect 461
      assert conn.status == 461
  ...
  describe "create phone_number" do
    ...
    # If regular users should be allowed to create a phone_number, then remove this test
  ...
  describe "update phone_number" do
    ...
    # If regular users should be allowed to create a phone_number, then remove this test
    test "won't work for regular user", %{conn: conn, phone_number: phone_number} do
      {:ok, conn, _user, _session} = Helpers.Accounts.regular_user_session_conn(conn)
      conn = put(conn, Routes.phone_number_path(conn, :update, phone_number), phone_number: @update_attrs)
      assert conn.status == 401
    end
    ...
    test "requires being authenticated", %{conn: conn, phone_number: phone_number} do
      conn = put(conn, Routes.phone_number_path(conn, :update, "42"), phone_number: @update_attrs)
      assert conn.status == 403
    end
