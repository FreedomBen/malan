defmodule Malan.Accounts.Session do
  ...
  schema "sessions" do
    ...
    # Maybe has_many :session_extension, SessionExtension
    # has_many :session_extensions, SessionExtension
    ...
    field :expire_in_seconds, :integer, virtual: true
  ...
  def extend_changeset(session, attrs) do
    session
    |> cast(attrs, [:expire_in_seconds])
    |> validate_expire_in_seconds()
    |> validate_required([:expire_in_seconds])
    |> validate_number(:expire_in_seconds, greater_than_or_equal_to: 0)
  ...
  defp validate_expire_in_seconds(changeset) do
    max_extension_secs = get_field(changeset, :max_extension_secs)

    case changeset do
      %{changes: %{expire_in_seconds: expire_in_seconds}} ->
        ...
        cond do
          expire_in_seconds <= max_extension_secs -> changeset
          true -> put_change(changeset, :expire_in_seconds, max_extension_secs)
        end

      _ ->
        # User didn't specify an extension time, so use the default max
        put_change(changeset, :expire_in_seconds, max_extension_secs)
    end
  end
  ...
  defp put_extension_expiration_time(changeset) do
    ...
    expire_in_seconds = get_change(changeset, :expire_in_seconds)
    ...
    new_expiration_time = Utils.DateTime.adjust_cur_time_trunc(expire_in_seconds, :seconds)
